...
export default function ProjectsCard({title, src, index}){
    ...
    return(
        ...
        >
            ...
            <p className='read-more' onClick={e => {index === 0 ? navigate('/works/minify') : navigate('/works/shortly'); e.preventDefault(); setHoverContent({...hoverContent, shouldBeDisabled: false, elementToRender: null, height: 25, width: 25})}} onMouseEnter={() => setHoverContent({...hoverContent ,shouldBeDisabled: true})} onMouseLeave={() => setHoverContent({...hoverContent, shouldBeDisabled: false})}>read more <TiltedArrow/></p>
        </Link>
    )
}
